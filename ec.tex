...
\end{figure}
\vspace{4cm}
The given Op-Amp is first order active low pass filter, so low frequency $\brak{\omega = 0}$ gain of given low-pass filter is,
\begin{figure}[ht]
	\centering
...
The above 	OP-Amp circuit becomes as non-inverting closed loop Op-Amp, so gain $\brak {A_V}$ is,
\begin{align}
A_V =& \lfloor 1 + \dfrac{R}{1} \rfloor \\
	=& 1+R
\end{align}
The $3-dB$ cut-off frequency of given low pass filter circuit is,
\begin{align}
\omega _{3dB} =& \dfrac{R_1}{C}  rad/s
\end{align}
...
\begin{figure}[ht]
    ...
    \label{fig: 2}
\end{figure}
Low frequency $\brak{\omega = 0}$ gain from bode magnitude plot is,
\begin{align}
\implies 12dB =& 20\text{ log}_{10}\brak{A_V} \\
A_V =& \text{Antilog} \lfloor \dfrac{12}{20} \rfloor \\
\implies 1+R =& 4\\
R =& 3k\ohm
\end{align}
The 3-dB frequency from bode magnitude plot,
\begin{align}
\implies \text{ log}_{10}\: \omega_{3dB}=& 3\\
\omega_{3dB} =& 1000 \: rad/sec 
\end{align}
\begin{align}
